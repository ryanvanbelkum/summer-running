...
const Weather = () => {
  ...
  const geoFetch = position => {
    fetch(
      ...
      }&lon=${position.coords.longitude}&APPID=${API_KEY}`
    ).then(rsp => {
  ...
  const stJoeFetch = () => {
    ...
    fetch(
      `https://api.openweathermap.org/data/2.5/weather?units=imperial&zip=64506,us&APPID=${API_KEY}`
    ).then(rsp => {
      rsp.json().then(data => setWeather(data));
    });
  };
  ...
  return (
    <div className="weather">
      ...
      <strong className="weather__temp">
